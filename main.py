...
URL_REMOTA = "https://raw.githubusercontent.com/ferpalaciosjl-wq/mi-app-gym/main/app_real.py"
...
# Funciones de almacenamiento local seguro en la carpeta de la App
def get_base_path():
    try:
    ...
    def preparar_dia(self, dia):
        ...
        for ej in ejercicios:
            ...
            for i in range(4):
                ...
                in_r = TextInput(hint_text="Reps", multiline=False, input_filter='int', halign='center')
        ...
        for fecha in sorted(datos.keys(), reverse=True):
            ...
            for ej, series in datos[fecha].items():
                if series:
                    ...
                    l = Label(text=f"[b]🏋️‍♂️ {ej}[/b]:\n      [color=B0BEC5]{txt}[/color]", markup=True, size_hint_y=None, height=dp(45), halign='left', text_size=(dp(300), None))
...
class EditarRutinaScreen(Screen):
    def __init__(self, **kwargs):
        ...
        rutinas = cargar_rutinas()
        self.spinner_dias = Spinner(text="👇 Selecciona el Día a Editar", values=list(rutinas.keys()), size_hint_y=None, height=dp(50), background_color=get_color_from_hex("#7E57C2"), bold=True)
        ...
        self.input_nuevo_ej = TextInput(hint_text="Escribe nuevo ejercicio...", multiline=False)
        ...
        btn_back = Button(text="💾 Guardar Rutina y Volver", size_hint_y=None, height=dp(55), background_color=get_color_from_hex("#1E88E5"), bold=True)
...
class GraficasScreen(Screen):
    ...
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        self.layout = BoxLayout(orientation='vertical', padding=dp(15), spacing=dp(10))
        self.spinner = Spinner(text="🔍 Selecciona Ejercicio", values=[], size_hint_y=None, height=dp(50), background_color=get_color_from_hex("#FB8C00"), bold=True)
...
# --- SISTEMA DE ACTUALIZACIÓN EN SEGUNDO PLANO SEGURA ---
class MainApp(App):
    def build(self):
        self.sm = ScreenManager()
        Window.clearcolor = get_color_from_hex("#121212")
        
        # Cargamos las pantallas reales integradas de inmediato
        self.sm.add_widget(MenuScreen(name='menu'))
        self.sm.add_widget(EntrenamientoScreen(name='entrenamiento'))
        self.sm.add_widget(HistorialScreen(name='historial'))
        sm_graficas = GraficasScreen(name='graficas')
        self.sm.add_widget(sm_graficas)
        self.sm.add_widget(EditarRutinaScreen(name='editar_rutina'))
        
        # Lanzamos la petición de actualización remota silenciosa sin congelar la app
        Clock.schedule_once(self.buscar_actualizacion_silenciosa, 2.0)
        return self.sm

    def buscar_actualizacion_silenciosa(self, dt):
        UrlRequest(URL_REMOTA, on_success=self.actualizar_codigo_local, timeout=5)

    def actualizar_codigo_local(self, req, resultado):
        # Si baja bien el archivo, lo guarda en la carpeta segura de la app de forma interna
        try:
            path_destino = os.path.join(get_base_path(), "app_real.py")
            with open(path_destino, "w", encoding="utf-8") as f:
                f.write(resultado)
        except:
            pass # Si Android bloquea la escritura, no pasa nada; la app sigue corriendo perfecta
