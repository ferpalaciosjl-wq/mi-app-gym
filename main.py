...
# --- CANVAS DE DIBUJO PARA GRÁFICAS ---
...
# --- COMPONENTES DE INTERFAZ (PANTALLAS) ---
...
class MenuScreen(Screen):
    ...
    def inicializar_menu(self):
        ...
        box.add_widget(Label(text="/// MI ENTRENAMIENTO ///", font_size=sp(22), bold=True, height=dp(50), size_hint_y=None, color=get_color_from_hex("#FFB300")))
        ...
        btn_graph = Button(text="[ Ver Gráficas de Avance ]", height=dp(58), size_hint_y=None, background_color=get_color_from_hex("#FB8C00"), bold=True, font_size=sp(16))
        btn_graph.bind(on_press=lambda x: setattr(self.manager, 'current', 'graficas'))
        box.add_widget(btn_graph)
        
        btn_edit_rutina = Button(text="[ Configurar Mis Rutinas ]", height=dp(58), size_hint_y=None, background_color=get_color_from_hex("#7E57C2"), bold=True, font_size=sp(16))
...
class HistorialScreen(Screen):
    ...
    def actualizar_historial(self):
        ...
        for fecha in sorted(datos.keys(), reverse=True):
            ...
            btn_borrar = Button(text="[ BORRAR ]", size_hint_x=None, width=dp(95), background_color=get_color_from_hex("#D32F2F"), font_size=sp(12), bold=True)
...
class EditarRutinaScreen(Screen):
    def __init__(self, **kwargs):
        ...
        self.layout.add_widget(Label(text="=== CONFIGURAR RUTINAS ===", font_size=sp(20), bold=True, size_hint_y=None, height=dp(40), color=get_color_from_hex("#7E57C2")))
        rutinas = cargar_rutinas()
        ...
        self.input_nuevo_ej = TextInput(hint_text="Nombre del nuevo ejercicio...", multiline=False, font_size=sp(15))
        btn_add = Button(text="[ + ]", size_hint_x=None, width=dp(60), background_color=get_color_from_hex("#43A047"), bold=True)
