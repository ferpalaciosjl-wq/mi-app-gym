...
# Usamos códigos de iconos unificados que no fallan en Android
...
class MenuScreen(Screen):
    ...
    def inicializar_menu(self):
        ...
        box.add_widget(Label(text="ENTRENAMIENTO TOTAL", font_size=sp(22), bold=True, height=dp(50), size_hint_y=None, color=get_color_from_hex("#FFB300")))
        for dia in rutinas.keys():
            btn = Button(text=f"o  {dia}", height=dp(55), size_hint_y=None, background_color=get_color_from_hex("#1E88E5"), bold=True, font_size=sp(15))
            btn.bind(on_press=lambda x, d=dia: self.ir_a_entreno(d))
            box.add_widget(btn)
        box.add_widget(Label(size_hint_y=None, height=dp(5)))
        
        btn_hist = Button(text="Historial en Tablas", height=dp(58), size_hint_y=None, background_color=get_color_from_hex("#43A047"), bold=True, font_size=sp(16))
        btn_hist.bind(on_press=lambda x: setattr(self.manager, 'current', 'historial'))
        box.add_widget(btn_hist)
        
        btn_graph = Button(text="Gráficas de Fuerza", height=dp(58), size_hint_y=None, background_color=get_color_from_hex("#FB8C00"), bold=True, font_size=sp(16))
        btn_graph.bind(on_press=lambda x: setattr(self.manager, 'current', 'graficas'))
        box.add_widget(btn_graph)
        
        btn_edit_rutina = Button(text="Configurar Rutinas", height=dp(58), size_hint_y=None, background_color=get_color_from_hex("#7E57C2"), bold=True, font_size=sp(16))
...
class EntrenamientoScreen(Screen):
    ...
    def preparar_dia(self, dia):
        ...
        if not ejercicios:
            self.container.add_widget(Label(text="-- No hay ejercicios --", halign='center', size_hint_y=None, height=dp(100)))
            return
...
class HistorialScreen(Screen):
    ...
    def actualizar_historial(self):
        ...
        if not datos:
            self.container.add_widget(Label(text="-- Historial Vacío --", halign='center', size_hint_y=None, height=dp(50)))
            return
        for fecha in sorted(datos.keys(), reverse=True):
            ...
            btn_borrar = Button(text="ELIMINAR", size_hint_x=None, width=dp(95), background_color=get_color_from_hex("#D32F2F"), font_size=sp(12), bold=True)
...
class EditarRutinaScreen(Screen):
    def __init__(self, **kwargs):
        ...
        self.layout.add_widget(Label(text="CONFIGURAR RUTA", font_size=sp(20), bold=True, size_hint_y=None, height=dp(40), color=get_color_from_hex("#7E57C2")))
        rutinas = cargar_rutinas()
        ...
        self.input_nuevo_ej = TextInput(hint_text="Nombre del ejercicio...", multiline=False, font_size=sp(15))
        btn_add = Button(text="MAS", size_hint_x=None, width=dp(60), background_color=get_color_from_hex("#43A047"), bold=True)
    ...
    def cargar_ejercicios_dia(self, spinner, text_dia):
        ...
        for ej in rutinas.get(text_dia, []):
            ...
            btn_del = Button(text="X", size_hint_x=None, width=dp(55), background_color=get_color_from_hex("#E53935"), bold=True)
    ...
    def anadir_ejercicio(self, instance):
        ...
        if not nombre or self.spinner_dias.text.startswith("Selecciona"): return
        fila = BoxLayout(orientation='horizontal', size_hint_y=None, height=dp(45), spacing=dp(5))
    ...
    def finalizar_edicion(self, instance):
        ...
        if not dia.startswith("Selecciona"):
            rutinas = cargar_rutinas()
...
class GraficasScreen(Screen):
    ...
    def __init__(self, **kwargs):
        ...
        btn_back = Button(text="< Volver al Menú", size_hint_y=None, height=dp(50), background_color=get_color_from_hex("#E53935"), bold=True)
    ...
    def actualizar_grafica(self, spinner, text):
        if text.startswith("Selecciona"): return
        ...
        if puntos_ejercicio:
            self.lbl_info.text = f"Fuerza Max en {text}: {round(puntos_ejercicio[-1][1],1)} kg (1RM)"
